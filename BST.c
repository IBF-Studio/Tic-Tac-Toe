...
treeNode* medianTraverse (treeNode *root1, int treeSize)
{
	while (treeSize)
	{
		if (root)
		{
			treeSize--;
			medianTraverse(root1->left, treeSize);
				return root1;
				medianTraverse(root1->right, treeSize);
...
void printGivenLevel(treeNode* root1, int level)
{
	if (root1 == NULL)
		return;
	if (level == 1)
